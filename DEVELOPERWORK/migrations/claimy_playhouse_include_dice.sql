-- Extend Playhouse feed + ranking stats to include Dice sessions (same metadata keys:
...
begin
  ...
  where g.game_key in ('flowerpoker', 'blackjack', 'dice')
    and (
      (
        v_wallet is null
...
comment on function public.playhouse_list_settled_bets(text, int, int) is
  'Playhouse: Flowerpoker + Blackjack + Dice. All settled when no wallet; wallet filter adds in_progress for that user.';
...
comment on function public.playhouse_player_ranking_stats(text) is
  'Settled-session aggregates (Flowerpoker + Blackjack + Dice): stake volume, PnL, W/L/T counts.';
