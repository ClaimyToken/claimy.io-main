...
| `dice-game` | `dice-game` | One-shot Dice: roll under/over on `0…9999`, HMAC-derived roll + credit settle in `claimy_game_sessions` (`game_key` `dice`). Action: `roll`. |
...
| `playhouse-feed` | `playhouse-feed` | Actions: **`list_bets`** (paginated feed; public = settled only; wallet filter adds in-progress for “My bets”) and **`player_ranking_stats`** (single JSON of SUM/COUNT for settled sessions — Flowerpoker + Blackjack + Dice). Run `DEVELOPERWORK/migrations/claimy_playhouse_feed.sql`, **`claimy_playhouse_player_ranking_stats.sql`**, **`claimy_playhouse_include_blackjack.sql`**, and **`claimy_playhouse_include_dice.sql`**. **`supabase/config.toml` sets `verify_jwt = false`**. Redeploy after RPC or handler changes. If `OPTIONS` returns 404, the slug is not deployed on that project. |
