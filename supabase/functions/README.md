...
| `playhouse-feed` | `playhouse-feed` | Actions: **`list_bets`** (paginated feed; public = settled only; wallet filter adds in-progress for “My bets”) and **`player_ranking_stats`** (single JSON of SUM/COUNT for settled Flowerpoker per wallet — Ranking progress). Run `DEVELOPERWORK/migrations/claimy_playhouse_feed.sql` and **`claimy_playhouse_player_ranking_stats.sql`**. **`supabase/config.toml` sets `verify_jwt = false`**. Redeploy after RPC or handler changes. If `OPTIONS` returns 404, the slug is not deployed on that project. |
